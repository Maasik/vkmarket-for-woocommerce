...
if ( ! class_exists( 'WP_Settings_API_Class2' ) ):
	class WP_Settings_API_Class2 {
		...
		function admin_init() {
			...
			foreach ( $this->settings_fields as $section => $field ) {
				foreach ( $field as $option ) {
					...
					add_settings_field( $section_tab[ $section ] . '[' . $option['name'] . ']', $option['label'], array(
						$this,
						'callback_' . $type
					), $section_tab[ $section ], $section, $args );
				}
			}
		...
		function callback_select_category_checklist( $args ) {
			...
			$defaults = array(
				'selected_cats' => $value,
				'walker'        => new WPSAPI_Walker_Checklist(), //new EVC_Walker_Checklist(),WPSAPI_Walker_Checklist()
				'taxonomy'      => apply_filters( 'wpsapi_taxonomy', 'category' ),
				'checked_ontop' => false
			);
		...
		function callback_select_product_checklist( $args ) {
			global $wc_name;
			...
			add_filter( 'wpsapi_checklist_name', array( $this, 'select_category_checklist_name' ) );
			add_filter( 'wpsapi_checklist_taxonomy', array( $this, 'wpsapi_checklist_taxonomy_product_cat' ) );
			...
			remove_filter( 'wpsapi_checklist_name', 'select_category_checklist_name' );
			remove_filter( 'wpsapi_checklist_taxonomy', 'wpsapi_checklist_taxonomy_product_cat' );
		...
		function wpsapi_checklist_taxonomy_product_cat() {
			return 'product_cat';
		}
		...
		function callback_wysiwyg( $args ) {
			...
			wp_editor( $value, $args['tab'] . '[' . $args['name'] . ']', array(
				'teeny'         => true,
				'textarea_rows' => 10
			) );
...
class WPSAPI_Walker_Checklist extends Walker {
	...
	function start_el( &$output, $object, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		$taxonomy = apply_filters( 'wpsapi_checklist_taxonomy', $taxonomy );
		/*
